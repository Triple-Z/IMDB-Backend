...
func ReadTitleDetails(c *gin.Context) {
	...
	rows, err := db.Query("select title_principals.id, title_basics.tconst, title_basics.id, title_principals.Ordering, name_basics.id, name_basics.nconst, name_basics.Primary_name, title_principals.Category, title_principals.Job, title_principals.Characters from title_basics join title_principals on title_basics.tconst = title_principals.tconst join name_basics on title_principals.nconst = name_basics.nconst where title_basics.id=? order by Ordering", id)
	...
	for rows.Next() {
		err := rows.Scan(&titleDetailSQL.Id, &titleDetailSQL.TConst, &titleDetailSQL.TitleId, &titleDetailSQL.Ordering, &titleDetailSQL.NameId, &titleDetailSQL.NConst, &titleDetailSQL.PrimaryName, &titleDetailSQL.Category, &titleDetailSQL.Job, &titleDetailSQL.Characters)
		if errCode := checkSQLError(err); errCode != 0 {
			switch errCode {
			case 1:
				c.JSON(http.StatusInternalServerError, gin.H{
